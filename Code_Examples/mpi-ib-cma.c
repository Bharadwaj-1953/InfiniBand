...
static int run_client(const char *hostip, const int port)
{
    ...
    if (ret) {
	fprintf(stderr, "%s: rdma_getaddrinfo: %d, %s \n", hostip, errno,
		gai_strerror(ret));
	goto out;
    }
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) {
	printf("Usage: %s <port_greater_than_18000>\n", argv[0]);
	return 1;
    }
    ...
    memcpy(hostip, ipaddr, strlen(ipaddr) + 1);

    if (size != 2) {
	fprintf(stderr, "\nInvalid number of processes for %s.\n",
		argv[0]);
	return 1;
    }

    if (rank == 0) {
	MPI_Recv(remoteip, 256, MPI_CHAR, 1, 111, MPI_COMM_WORLD,
		 MPI_STATUS_IGNORE);

	run_client(remoteip, server_port + 5);

	struct sockaddr_in server_addr;
	server_addr.sin_family = AF_INET;
	server_addr.sin_port = htons(server_port);
	if (inet_pton(AF_INET, remoteip, &server_addr.sin_addr) <= 0) {
	    fprintf(stderr,
	...
	if (ret) {
	    fprintf(stderr, "%d: failed to connect %s at port %d\n",
		    __LINE__, argv[1], server_port);
	}
	// Cleanup connection
	close(sock);
	fprintf(stderr, "rank %d line %d: closed sockets.\n", rank,
		__LINE__);
    } else {

	MPI_Send(hostip, 256, MPI_CHAR, 0, 111, MPI_COMM_WORLD);
