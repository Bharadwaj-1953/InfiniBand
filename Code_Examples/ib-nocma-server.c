...
int main(int argc, char **argv)
{
    ...
    send(client_sock, &my_info, sizeof(my_info), 0);
    recv(client_sock, &peer_info, sizeof(peer_info), 0);

    if (cn_connect_ctx(ctx, DHT_IB_PORT, my_info.psn, &peer_info))
	return 1;

    // Wait for client notification on completion
    char mesg[64];
    recv(client_sock, mesg, 64, 0);
