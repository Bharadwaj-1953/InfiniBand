...
int main(int argc, char **argv)
{
    ...
    struct conn_context *ctx;
    struct conn_context *cn_ctx[2];
    struct conn_info my_info, cn_info[2];
    ...
    /* need to create size-1 ctx and qp */
    for (int i = 1; i < size; i++) {
	int peer = (rank + i) % size;
	...
	cn_ctx[peer] = ctx;
	/* stash the information for the array */
	memcpy(&cn_info[peer], &my_info, sizeof(my_info));
    }

    struct conn_info *all_cn_info =
	(struct conn_info *) malloc(sizeof(struct conn_info) * size *
				    size);

    for (int i = 1; i < size; i++) {
	int peer = rank + i;

	int idx = size * peer + rank;	/* my rank in the peer-th block */
	struct conn_info *rem_cn_info = all_cn_info + idx;

	// establish connection with all remote destinations
	if (cn_connect_ctx
	    (&ctx[peer], DHT_IB_PORT, cn_info[peer].psn, rem_cn_info))
	    return 1;
    }
